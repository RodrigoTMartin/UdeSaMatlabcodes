...
// Parameters 
parameters SIG VPHI CHI BET GAM VRHO NU XI GAMMA;
...
exp(cW)+exp(mW)+exp(tao)=exp(w)*exp(hW)+exp(mW(-1))/exp(ppi) + exp(rer)*(GAMMA*exp(yT));                   //E4
...
exp(Rst(-1))/exp(ppist)*exp(dst(-1)) = tb - (GAMMA)*exp(yT)*(1-DELTA) + exp(dst);                             //E25
...
tb_ss       =  dst_ss*(Rst_ss/ppist_ss-1) + yT_ss*(GAMMA)*(1-DELTA);
...
cW_ss       =w_ss*hW_ss-tao_ss + rer_ss*(GAMMA*yT_ss);
...
save TANK_T_Cugat2.mat oo_ M_ options_;
