...
var c la h w Rst ppiS ppi R cN pN p cT ppiT rer ppiN mcN yN m CN;
var gdp dst tb;

// Variables that hav associated an exogenous process
var z ppist em Rw yT ppiTar gEl;
...
parameters R_ss gdp_ss ppi_ss z_ss ppist_ss em_ss Rw_ss yT_ss ngdp_ss g_ss; // in eqns and exon
parameters h_ss stb_ss rer_ss sm_ss syT_ss sg_ss;                      // given in ss
...
exp(cN) = exp(yN)*(1-PHIN/2*(exp(ppiN)-1)^2)-exp(g);                  //modificada

// National identities and definitions 4 eqns
exp(Rst(-1))/exp(ppist)*dst(-1) = tb + dst;                                                           //E19modificada

exp(gdp) = exp(c) + tb + exp(g);                                                                                           // E21
...
g-log(g_ss)          = RHO_g*(g(-1)-log(g_ss))+u_g;          //NUEVA
end; 
...
save TNT_NK_2_baseline1gasto.mat oo_ M_ options_;
