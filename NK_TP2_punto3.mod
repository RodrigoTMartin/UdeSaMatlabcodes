...
var c laC laW h w Rst ppiS ppi R cN pN p cT ppiT rer ppiN mcN yN m cW cC hW dstC mW tao d dst;
var gdp tb;
...
parameters DELTA PHID PHIG DEUDA PHITy;            //NUEVOS
...
exp(dst)=(1-DELTA)*exp(dstC);                                           //E11
//exp(d)=(1-DELTA)*exp(dC);                                               //E12

//Ecuaciones de producción total
exp(cN) = GAM*(exp(pN)/exp(p))^(-VRHO)*exp(c);                          //E13
exp(cT) = (1-GAM)*(1/exp(p))^(-VRHO)*exp(c)+PHITy;                            //E14
exp(c) = (GAM^(1/VRHO)*exp(cN)^(1-1/VRHO) + (1-GAM)^(1/VRHO)*(exp(cT)-PHITy)^(1-1/VRHO))^(VRHO/(VRHO-1));   //E15
...
exp(Rst) = exp(Rw)*exp(PHI_B*(exp(dst)-BARD));                               //E19
...
exp(Rst(-1))/exp(ppist)*exp(dst(-1)) = tb + exp(dst);                             //E25
...
exp(R(-1))*exp(d(-1))/exp(ppi)+exp(m)+exp(tao)=exp(d)+exp(m(-1))/exp(ppi(-1))+exp(pN)*exp(g);                     //E29
(exp(tao)-tao_ss)/gdp_ss = PHID*(((-1)*exp(d)+d_ss)/gdp_ss)+PHIG*(((exp(pN)*exp(g))-(exp(pN)*g_ss))/gdp_ss);      //E30
//IM = (gdp(+1)-gdp) / (exp(g(+1))-exp(g));
...
c_ss        = (GAM^(1/VRHO)*cN_ss^(1-1/VRHO) + (1-GAM)^(1/VRHO)*(cT_ss-PHITy)^(1-1/VRHO))^(VRHO/(VRHO-1));

rer_ss      = ((cT_ss-PHITy)/(c_ss*(1-GAM)))^(-1/VRHO);
...
save TNT_NK_2_peg_ej1b.mat oo_ M_ options_;
